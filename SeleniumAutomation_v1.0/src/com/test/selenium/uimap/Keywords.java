	package com.test.selenium.uimap;
	...
	*/
	...
	import static com.test.selenium.testscript.TestDriver.APP_LOGS;
import static com.test.selenium.testscript.TestDriver.CONFIG;
import static com.test.selenium.testscript.TestDriver.UIMap;
import static com.test.selenium.testscript.TestDriver.testResults;
...
import com.test.selenium.testscript.BarChart;
import com.test.selenium.testscript.Constants;
import com.test.selenium.testscript.LineChart;
import com.test.selenium.testscript.PieChart;
import com.test.selenium.testscript.RestAPIUtil;
import com.test.selenium.testscript.Table;
import com.test.selenium.utils.DBUtil;
import com.test.selenium.utils.TestUtil;
import com.google.common.base.Function;
import com.thoughtworks.selenium.Selenium; //download selenium-java-client-driver-1.0.2.jar
import com.thoughtworks.selenium.DefaultSelenium;

	//import com.sikuli.ImageElement;
	//import com.sikuli.SikuliFirefoxDriver;
	...
	############################################################################## */
		...
		public String openBrowser(String object,String data) {
			...
			if(data.equalsIgnoreCase("firefox")){
				if(prof==null){
					driver=new FirefoxDriver(); //commented on June 13 2017
					//System.setProperty("webdriver.chrome.driver","Users/ayeesri/Downloads/chromedriver.exe");
		...
		*/
		...
		public String setListItem(String object, String item){
			...
			if(select != null){
				...
				try{
					WebElement option = select.findElement(By.xpath("//option[contains(text(),'" + item + "')]"));
					option.click();
					return Constants.KEYWORD_PASS;
		...
		public  String verifyCheckBoxSelected(String object,String data){
			...
			if(element != null){
				...
				if(checked!=null){
					...
					return Constants.KEYWORD_FAIL + " checkbox "+locator+" - Not selected";
				}
			}
			else{
				APP_LOGS.debug(Constants.KEYWORD_FAIL+" -- Could not find checkbox -- Element "+locator+" Not Found");
		...
		*/
			...
			for(int i=0;i<childObjects.length;i++){
				...
				for(int j=0;j<ele_list.size();j++){
					...
					if(ele_list.get(j).getTagName().equals("input")){
						...
						ele_list.get(j).sendKeys(dataInput[j]);
					}else if(inputLocator.toLowerCase().contains("select")){
						ele_list.get(j).findElement(By.xpath("//option[text()='" + dataInput[j] + "']")).click();
					}
				}
			}
			return Constants.KEYWORD_PASS;
		...
		public String tableOperation(String object,String data){
			...
			for(String child: childObjects){
				...
				}else if(UIMap.getProperty(child).contains("tbody")){
			...
			*/
			System.out.println("hello subtask "+subTask);
			System.out.println("hello childObject "+childObject);
			String locator = table.replace("ROW", "1").replace("COLUMN", "1");
			System.out.println("hello table "+locator);
			try {
				
				if(waitForElement(locator)){
					System.out.println("hello subtask again "+subTask);
					...
					if(subTask.toLowerCase().contains("clickcheckbox")){
						...
						System.out.println("VerifyCellData "+cellData);
		...
		/***public String testLineChart(String object,String data){
			...
			if(chart != null){
				...
				try{
					 ...
					 HashMap<Integer, com.testscript.Point> pp = lc.getSeriesPoints();
		...
		public String testBarChart(String object,String data){
